  ...
  const generateShortUrl = async () => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/url/shorten`, {
        ...
        body: JSON.stringify({
          longUrl,
          userId: "34355", // This should be dynamically set in a real application
          metadata: {
            title: "URL Shortener",
      ...
      setUrlHistory([{ longUrl, shortenedUrl: data.shortenURL }, ...urlHistory])
      setLongUrl("")
      copyToClipboard(data.shortUrl) // Automatically copy the new short URL
      setCopying(data.shortUrl)
      setTimeout(() => setCopying(null), 2000)
      toast.success(`Short URL generated and copied to clipboard: ${data.shortUrl}`)
    } catch (error) {
